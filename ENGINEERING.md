...
filtering). Cross-origin iframes are silently skipped by `getAllFrames()`. Each
...
descendants, reported as `{ type, description, inViewport, formState, index }` —
...
export function getElementInventory(parent = null)
  → Array<{
       ...
       overlay: { type: string, description: string | null, inViewport: boolean, formState: Object | null, index: number } | null
     }>
```
...
- The `index` for an **identified** element (one with text) is the 1-based
  ...
  **text-less** element (no text and no nearby label) is the positional `#N` —
  ...
  `findElements(type, null)` returns. The `overlay` entry's `index` mirrors that
